...
static bool s_bit_mask32(const DVVectorLike& vec, const DVVector& dv_min, size_t begin, size_t end, uint32_t& bit_mask)
{
	DVSizeT dvbegin(begin);

	Functor src({ {"vec", &vec}, {"begin", &dvbegin }, {"v_min", &dv_min } }, { "idx" },
	...
	ViewBuf buf;
	if (!general_reduce(end - begin, "uint32_t", src, op, buf)) return false;
	bit_mask = *(uint32_t*)buf.data();

	return true;
}

static bool s_bit_mask64(const DVVectorLike& vec, const DVVector& dv_min, size_t begin, size_t end, uint64_t& bit_mask)
{
	DVSizeT dvbegin(begin);
	...
	ViewBuf buf;
	if (!general_reduce(end - begin, "uint64_t", src, op, buf)) return false;
...
static bool s_partition_scan_32(size_t n, const DVVector& src, const DVVector& dv_min, int bit, DVVector& inds, uint32_t& count)
{
	...
	if (!general_scan(n, src_scan, inds, plus, 0)) return false;
...
bool radix_sort_32(DVVectorLike& vec, size_t begin, size_t end)
{
	if (end == (size_t)(-1)) end = vec.size();

	size_t id_min;
	if (!TRTC_Min_Element(vec, id_min, begin, end)) return false;

	DVVector dv_min(vec.name_elem_cls().c_str(), 1);
	TRTC_Copy(vec, dv_min, id_min, id_min + 1);

	uint32_t bit_mask;
	if (!s_bit_mask32(vec, dv_min, begin, end, bit_mask)) return false;
	if (bit_mask == 0) return true; // already sorted

	size_t n = end - begin;
	DVVector tmp1(vec.name_elem_cls().c_str(), n);
	DVVector tmp2(vec.name_elem_cls().c_str(), n);
	if (!TRTC_Copy(vec, tmp1, begin, end)) return false;
	DVVector* p1 = &tmp1;
	DVVector* p2 = &tmp2;
	...
	if (!TRTC_Copy(*p1, vec, 0, n, begin)) return false;
	return true;
}

bool radix_sort_reverse_32(DVVectorLike& vec, size_t begin, size_t end)
{
	if (end == (size_t)(-1)) end = vec.size();
...
bool radix_sort_64(DVVectorLike& vec, size_t begin, size_t end)
{
	if (end == (size_t)(-1)) end = vec.size();
	...
	uint64_t bit_mask;
	if (!s_bit_mask64(vec, dv_min, begin, end, bit_mask)) return false;
	if (bit_mask == 0) return true; // already sorted
...
bool radix_sort_reverse_64(DVVectorLike& vec, size_t begin, size_t end)
{
	if (end == (size_t)(-1)) end = vec.size();
...
bool radix_sort_by_key_32(DVVectorLike& keys, DVVectorLike& values, size_t begin_keys, size_t end_keys, size_t begin_values)
{
	...
	size_t id_min;
	if (!TRTC_Min_Element(keys, id_min, begin_keys, end_keys)) return false;

	DVVector dv_min(keys.name_elem_cls().c_str(), 1);
	TRTC_Copy(keys, dv_min, id_min, id_min + 1);

	uint32_t bit_mask;
	if (!s_bit_mask32(keys, dv_min, begin_keys, end_keys, bit_mask)) return false;
	if (bit_mask == 0) return true; // already sorted

	size_t n = end_keys - begin_keys;
	DVVector tmp_keys1(keys.name_elem_cls().c_str(), n);
	DVVector tmp_keys2(keys.name_elem_cls().c_str(), n);
	DVVector tmp_values1(values.name_elem_cls().c_str(), n);
	DVVector tmp_values2(values.name_elem_cls().c_str(), n);
	if (!TRTC_Copy(keys, tmp_keys1, begin_keys, end_keys)) return false;
	if (!TRTC_Copy(values, tmp_values1, begin_values, begin_values + n)) return false;
	...
	for (int bit = 0; bit < 32; bit++)
	{
		...
		uint32_t count;
		if (!s_partition_scan_32(n, *p_keys1, dv_min, bit, inds, count)) return false;
		if (!s_partition_scatter_by_keys(n, *p_keys1, *p_values1, inds, *p_keys2, *p_values2, count)) return false;
	...
	if (!TRTC_Copy(*p_keys1, keys, 0, n, begin_keys)) return false;
	if (!TRTC_Copy(*p_values1, values, 0, n, begin_values)) return false;
...
bool radix_sort_by_key_reverse_32(DVVectorLike& keys, DVVectorLike& values, size_t begin_keys, size_t end_keys, size_t begin_values)
{
	...
	for (int bit = 0; bit < 32; bit++)
	{
		...
		uint32_t count;
		if (!s_partition_scan_reverse_32(n, *p_keys1, dv_min, bit, inds, count)) return false;
		if (!s_partition_scatter_by_keys(n, *p_keys1, *p_values1, inds, *p_keys2, *p_values2, count)) return false;
...
bool radix_sort_by_key_64(DVVectorLike& keys, DVVectorLike& values, size_t begin_keys, size_t end_keys, size_t begin_values)
{
	...
	uint64_t bit_mask;
	if (!s_bit_mask64(keys, dv_min, begin_keys, end_keys, bit_mask)) return false;
	if (bit_mask == 0) return true; // already sorted
	...
	for (int bit = 0; bit < 64; bit++)
	{
		...
		uint32_t count;
		if (!s_partition_scan_64(n, *p_keys1, dv_min, bit, inds, count)) return false;
		if (!s_partition_scatter_by_keys(n, *p_keys1, *p_values1, inds, *p_keys2, *p_values2, count)) return false;
...
bool radix_sort_by_key_reverse_64(DVVectorLike& keys, DVVectorLike& values, size_t begin_keys, size_t end_keys, size_t begin_values)
{
	...
	for (int bit = 0; bit < 64; bit++)
	{
		...
		uint32_t count;
		if (!s_partition_scan_reverse_64(n, *p_keys1, dv_min, bit, inds, count)) return false;
		if (!s_partition_scatter_by_keys(n, *p_keys1, *p_values1, inds, *p_keys2, *p_values2, count)) return false;
