...
bool s_reduce(TRTCContext& ctx, const DVVector& src, DVVector& res, const Functor& op)
{
	...
	return ctx.launch_kernel({ blocks,1,1 }, { BLOCK_SIZE ,1,1 }, arg_map,
		(std::string("    extern __shared__ ") + src.name_elem_cls() + " _s_buf[];\n"
			...
			"    " + src.name_elem_cls() + "& _here=_s_buf[_tid];\n"
			"    if (_i<_n)\n"
			"        _here = _view_src[_i];\n"
			"    __syncthreads();\n"
			"    for (unsigned _s = blockDim.x/2; _s>0; _s>>=1)\n    {\n"
			"        if (_tid < _s && _i+_s<_n)\n        {\n" + op.generate_code(src.name_elem_cls().c_str(), { "_here", "_s_buf[_tid + _s]" }) +
			"            _here = " + op.functor_ret + ";\n        }\n"
			"        __syncthreads();\n    }\n"
...
bool general_reduce(TRTCContext& ctx, size_t n, const char* name_cls, const Functor& src, const Functor& op, ViewBuf& ret_buf)
{
	...
	// first round
	{
		...
		std::vector<TRTCContext::AssignedParam> arg_map(src.arg_map.size() + op.arg_map.size() + 2);
		memcpy(arg_map.data(), src.arg_map.data(), src.arg_map.size() * sizeof(TRTCContext::AssignedParam));
		memcpy(arg_map.data() + src.arg_map.size(), op.arg_map.data(), op.arg_map.size() * sizeof(TRTCContext::AssignedParam));
		TRTCContext::AssignedParam* p_arg_map = &arg_map[src.arg_map.size() + op.arg_map.size()];
		...
		if (!ctx.launch_kernel({ blocks,1,1 }, { BLOCK_SIZE ,1,1 }, arg_map,
			(std::string("    extern __shared__ ") + name_cls + " _s_buf[];\n"
				...
				"    __syncthreads();\n"
				"    for (unsigned _s = blockDim.x/2; _s>0; _s>>=1)\n    {\n"
				"        if (_tid < _s && _i+_s<_n)\n        {\n" + op.generate_code(name_cls, { "_here", "_s_buf[_tid + _s]" }) +
				"            _here = " + op.functor_ret + ";\n        }\n"
				"        __syncthreads();\n    }\n"
				"    if (_tid==0) _view_res[blockIdx.x] = _here;"
				).c_str(),
