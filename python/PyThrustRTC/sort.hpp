...
static PyObject* n_is_sorted(PyObject* self, PyObject* args)
{
	...
	if (comp == nullptr)
	{
		if (TRTC_Is_Sorted(*ctx, *vec, res, begin, end))
	...
	{
		if (TRTC_Is_Sorted(*ctx, *vec, *comp, res, begin, end))
			return PyBool_FromLong(res ? 1 : 0);
		else
			Py_RETURN_NONE;
	}
...
static PyObject* n_is_sorted_until(PyObject* self, PyObject* args)
{
	...
	if (comp == nullptr)
	{
		if (TRTC_Is_Sorted_Until(*ctx, *vec, res, begin, end))
	...
	{
		if (TRTC_Is_Sorted_Until(*ctx, *vec, *comp, res, begin, end))
			return PyLong_FromUnsignedLongLong((unsigned long long)res);
		else
			Py_RETURN_NONE;
	}
