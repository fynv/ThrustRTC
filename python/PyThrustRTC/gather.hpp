...
static PyObject* n_gather(PyObject* self, PyObject* args)
{
	...
	size_t begin_map = (size_t)PyLong_AsLong(PyTuple_GetItem(args, 3));
	size_t end_map = (size_t)PyLong_AsLong(PyTuple_GetItem(args, 4));
	size_t begin_in = (size_t)PyLong_AsLong(PyTuple_GetItem(args, 5));
	size_t begin_out = (size_t)PyLong_AsLong(PyTuple_GetItem(args, 6));
	if (TRTC_Gather(*vec_map, *vec_in, *vec_out, begin_map, end_map, begin_in, begin_out))
...
static PyObject* n_gather_if(PyObject* self, PyObject* args)
{
	...
	size_t end_map = (size_t)PyLong_AsLong(PyTuple_GetItem(args, 6));
	size_t begin_stencil = (size_t)PyLong_AsLong(PyTuple_GetItem(args, 7));
	size_t begin_in = (size_t)PyLong_AsLong(PyTuple_GetItem(args, 8));
	size_t begin_out = (size_t)PyLong_AsLong(PyTuple_GetItem(args, 9));
	if (pred == nullptr)
	{
		if (TRTC_Gather_If(*vec_map, *vec_stencil, *vec_in, *vec_out, begin_map, end_map, begin_stencil, begin_in, begin_out))
			return PyLong_FromLong(0);
		else
			Py_RETURN_NONE;
	}
	...
	{
		if (TRTC_Gather_If(*vec_map, *vec_stencil, *vec_in, *vec_out, *pred, begin_map, end_map, begin_stencil, begin_in, begin_out))
			return PyLong_FromLong(0);
		else
			Py_RETURN_NONE;
	}
