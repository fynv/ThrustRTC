...
{
	TRTCContext* ctx = (TRTCContext*)PyLong_AsVoidPtr(PyTuple_GetItem(args, 0));
	DVVectorLike* vec_key = (DVVectorLike*)PyLong_AsVoidPtr(PyTuple_GetItem(args, 1));
	DVVectorLike* vec_value = (DVVectorLike*)PyLong_AsVoidPtr(PyTuple_GetItem(args, 2));
	...
	{
		if (TRTC_Inclusive_Scan_By_Key(*ctx, *vec_key, *vec_value, *vec_out, begin_key, end_key, begin_value, begin_out))
			return PyLong_FromLong(0);
		Py_RETURN_NONE;
	}
	else if (binary_op == nullptr)
	{
		if (TRTC_Inclusive_Scan_By_Key(*ctx, *vec_key, *vec_value, *vec_out, *binary_pred, begin_key, end_key, begin_value, begin_out))
			return PyLong_FromLong(0);
		Py_RETURN_NONE;
	}
	else
	{
		if (TRTC_Inclusive_Scan_By_Key(*ctx, *vec_key, *vec_value, *vec_out, *binary_pred, *binary_op, begin_key, end_key, begin_value, begin_out))
			return PyLong_FromLong(0);
		Py_RETURN_NONE;
	}
}
...
static PyObject* n_exclusive_scan_by_key(PyObject* self, PyObject* args)
{
	...
	if (py_binary_op != Py_None)
		binary_op = (Functor*)PyLong_AsVoidPtr(py_binary_op);
	size_t begin_key = (size_t)PyLong_AsLong(PyTuple_GetItem(args, 7));
	size_t end_key = (size_t)PyLong_AsLong(PyTuple_GetItem(args, 8));
	size_t begin_value = (size_t)PyLong_AsLong(PyTuple_GetItem(args, 9));
	size_t begin_out = (size_t)PyLong_AsLong(PyTuple_GetItem(args, 10));

	if (init == nullptr)
	{
		if (TRTC_Exclusive_Scan_By_Key(*ctx, *vec_key, *vec_value, *vec_out, begin_key, end_key, begin_value, begin_out))
			return PyLong_FromLong(0);
		Py_RETURN_NONE;
	}
	if (binary_pred == nullptr)
	{
		if (TRTC_Exclusive_Scan_By_Key(*ctx, *vec_key, *vec_value, *vec_out, *init, begin_key, end_key, begin_value, begin_out))
			return PyLong_FromLong(0);
		Py_RETURN_NONE;
	}
	else if (binary_op == nullptr)
	{
		if (TRTC_Exclusive_Scan_By_Key(*ctx, *vec_key, *vec_value, *vec_out, *init, *binary_pred, begin_key, end_key, begin_value, begin_out))
			return PyLong_FromLong(0);
		Py_RETURN_NONE;
	}
	else
	{
		if (TRTC_Exclusive_Scan_By_Key(*ctx, *vec_key, *vec_value, *vec_out, *init, *binary_pred, *binary_op, begin_key, end_key, begin_value, begin_out))
			return PyLong_FromLong(0);
		Py_RETURN_NONE;
	}
}
