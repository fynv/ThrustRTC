...
class For:
    def __init__(self, param_names, name_iter, body):
        o_param_names = StrArray(param_names)
        self.m_cptr = native.n_for_create(o_param_names.m_cptr, name_iter.encode('utf-8'), body.encode('utf-8'))
    ...
    def launch(self, begin, end, args):
        arg_list = ObjArray(args)
        native.n_for_launch(self.m_cptr, begin, end, arg_list.m_cptr)

    def launch_n(self, n, args):
        arg_list = ObjArray(args)
        native.n_for_launch_n(self.m_cptr, n, arg_list.m_cptr)
