...
class DVConstant(DVVectorLike):
	def __init__(self, dvobj, size = -1):
		self.m_dvobj = dvobj
		self.m_cptr = native.n_dvconstant_create(dvobj.m_cptr, ctypes.c_ulonglong(size).value)

class DVCounter(DVVectorLike):
	def __init__(self, dvobj_init, size = -1):
		self.m_dvobj_init = dvobj_init
		self.m_cptr = native.n_dvcounter_create(dvobj_init.m_cptr, ctypes.c_ulonglong(size).value)

class DVDiscard(DVVectorLike):
	def __init__(self, elem_cls, size = -1):
		self.m_cptr = native.n_dvdiscard_create(elem_cls.encode('utf-8'), ctypes.c_ulonglong(size).value)
...
class DVTransform(DVVectorLike):
	def __init__(self, vec_in, elem_cls, op):
		...
		self.m_op = op
		self.m_cptr = native.n_dvtransform_create(vec_in.m_cptr, elem_cls.encode('utf-8'), op.m_cptr)
...
class DVZipped(DVVectorLike):
	def __init__(self, vecs, elem_names):
		...
		self.m_cptr = native.n_dvzipped_create(o_vecs.m_cptr, o_elem_names.m_cptr)
