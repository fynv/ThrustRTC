...
int main()
{
...

	{
		...
		TRTC_Scatter(d_values, d_map, d_output);
...

	{
		...
		DVVector d_V("int32_t", 8, h_V);

		int h_M[8] = { 0, 5, 1, 6, 2, 7, 3, 4 };
		DVVector d_M("int32_t", 8, h_M);

		int h_S[8] = { 1, 0, 1, 0, 1, 0, 1, 0 };
		DVVector d_S("int32_t", 8, h_S);

		int h_D[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
		DVVector d_D("int32_t", 8, h_D);

		TRTC_Scatter_If(d_V, d_M, d_S, d_D);
		d_D.to_host(h_D);
...

	{
		...
		TRTC_Scatter_If(d_V, d_M, d_S, d_D, is_even);
		d_D.to_host(h_D);
