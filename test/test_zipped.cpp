...
int main()
{
	TRTCContext ctx;
	{
		...
		DVZipped zipped_in(ctx, { &d_int_in, &d_float_in }, { "a", "b" });
		DVZipped zipped_out(ctx, { &d_int_out, &d_float_out }, { "a", "b" });

		TRTC_Copy(ctx, zipped_in, zipped_out);
	...
	{
		DVCounter d_int_in(ctx, DVInt32(0), 5);
		DVTransform d_float_in(ctx, d_int_in, "float", Functor(ctx, {}, { "i" }, "        return (float)i*10.0f +10.0f;\n"));
...

	{
		...
		DVTuple d_tuple(ctx, { {"a", &d_int_in}, {"b",&d_float_in} });
		DVConstant const_in(ctx, d_tuple, 5);
		...
		TRTC_Copy(ctx, const_in, zipped_out);
		d_int_out.to_host(h_int_out);
		d_float_out.to_host(h_float_out);
