...
int main()
{
...

	{
		...
		TRTC_Transform(ctx, vec, vec, Functor("Negate")); // in place
		vec.to_host(hvalues);
...

	{
		...
		TRTC_Transform_Binary(ctx, d_in1, d_in2, d_out, Functor("Plus"));
...

	{
		int hvalues[10] = { -5, 0, 2, -3, 2, 4, 0, -1, 2, 8 };
		DVVector vec(ctx, "int32_t", 10, hvalues);
		TRTC_Transform_If(ctx, vec, vec, Functor("Negate"), is_odd); // in place
		vec.to_host(hvalues);
		printf("%d %d %d %d %d ", hvalues[0], hvalues[1], hvalues[2], hvalues[3], hvalues[4]);
		printf("%d %d %d %d %d\n", hvalues[5], hvalues[6], hvalues[7], hvalues[8], hvalues[9]);
	}

	{
		...
		TRTC_Transform_If_Stencil(ctx, d_data, d_stencil, d_data, Functor("Negate"), Functor("Identity")); // in place
		d_data.to_host(data);
...

	{
		...
		TRTC_Transform_Binary_If_Stencil(ctx, d_in1, d_in2, d_stencil, d_output, Functor("Plus"), Functor("Identity")); // in place
